...
{
	printf("\n\n");
	system("color 1a");//color 1a  1是背景颜色 a是字体颜色
...
void Change()//修改信息
{
	...
	struct ima c[20];//创建结构体数组一个一个拿到结构体里面的数据
	...
	for (i = 0; i < index; i++)//i<index是到读到的个数
	{
		if (name == c[i].id)//如果编号相同
		{
			...
			ref:
				printf("请输入新的编号是:\n");
				scanf("%d", &c[i].id);
				if (validateID(c[i].id) == 1)//验证编号是否重复，重复重新输入
				{
					printf("\t\t\t\t此编号已被使用，请您重新输入......\n");
					goto ref;//goto语句跳转
				}
				/*if (determine(c[i].id) == 1)
				{
					system("pause");
					goto again;
				}*/
